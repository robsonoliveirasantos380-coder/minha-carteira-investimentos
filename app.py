...
# Função auxiliar para gerar arquivo Excel
def to_excel(df1, df2, ticker1, ticker2):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df1.to_excel(writer, sheet_name=ticker1[:10])
        df2.to_excel(writer, sheet_name=ticker2[:10])
    return output.getvalue()

# --- SÓ CARREGA O APLICATIVO SE O LOGIN FOR VÁLIDO ---
if check_password():
    ...
    with col_t1:
        ticker1 = st.sidebar.text_input("Ativo 1:", value="MXRF11.SA").upper()
    with col_t2:
        ticker2 = st.sidebar.text_input("Ativo 2:", value="KNCA11.SA").upper()
